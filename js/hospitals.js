/* ============================================================
   ...
   Vercel Production Optimized Version
   Routes all Overpass queries through /api/hospitals
   ============================================================ */
...
(function () {
  ...
  function useGeolocation(silent) {
    ...
    if (btn) {
      ...
      if (window.HBA_refreshIcons) window.HBA_refreshIcons();
    }
  ...
  async function ipFallback(silent) {
    const providers = [
      async () => {
        ...
        return { lat: d.latitude, lng: d.longitude, label: `${d.city || ''} ${d.country || ''}`.trim() };
      },
      async () => {
        const r = await fetch('https://get.geojs.io/v1/ip/geo.json');
        if (!r.ok) throw 0;
        const d = await r.json();
        return { lat: parseFloat(d.latitude), lng: parseFloat(d.longitude), label: `${d.city || ''} ${d.country || ''}`.trim() };
      }
    ];
    ...
    for (const p of providers) {
      try {
        ...
        if (lat && lng) {
          applyCenter(lat, lng, 'Approximate: ' + (label || 'your area'));
          return;
        }
      } catch (_) {}
    }
    ...
    if (!silent) {
      renderError('Could not detect your location. Please type a city in the search box.');
    } else {
      loadPlaces(lastCenter.lat, lastCenter.lng);
    }
  }
  ...
  function applyCenter(lat, lng, label) {
    ...
    userMarker = L.marker([lat, lng], { title: label }).addTo(map).bindPopup(label).openPopup();
    loadPlaces(lat, lng);
  }
  ...
  async function loadPlaces(lat, lng) {
    ...
    try {
      /* SURGICAL FIX: Using the Vercel internal API route to bypass CORS */
      const response = await fetch(`/api/hospitals?query=${encodeURIComponent(query)}`);
      ...
      console.error(e);
      renderError('Could not load nearby places.');
    }
  }
  ...
  function renderPlaces(places) {
    ...
    places.forEach((p, i) => {
      ...
      marker.bindPopup(`<strong>${escapeHtml(p.name)}</strong><br>${p.distance.toFixed(2)} km away`);
  ...
  function renderError(msg) {
    const list = document.getElementById('placesList');
    if (list) {
      list.innerHTML = `<div class="empty-state"><i data-lucide="map-off"></i><p>${msg}</p></div>`;
      if (window.HBA_refreshIcons) window.HBA_refreshIcons();
    }
  }
